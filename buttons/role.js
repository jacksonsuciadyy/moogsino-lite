module.exports = {
    ...
    run: async (bot, interaction, parameters) => {
        ...
        if(member.roles.cache.has(role.id)) {
            ...
            return interaction.reply({content: `Removed the role ${role.name} from you!`, ephremeral: true})
        } else {
        // member doesn't have the role => add role
            await member.roles.add(role.id)
            return interaction.reply({content: `Added the role ${role.name} to you!`, ephremeral: true})
        }
